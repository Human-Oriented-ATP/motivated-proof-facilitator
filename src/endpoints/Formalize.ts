...
export const formalizeStatementServer = async (naturalLanguageStatement: string): Promise<ProofState> => {
...
Please adjust your approach to avoid this error.`;
  ...
  throw new Error("Failed to formalize statement after retries");
}
...
export async function formalizeStatement({ problem }: { problem: string }): Promise<ProofState> {
  ...
  return ProofStateSchema.parse(data)
}